            ...
            while ((record = sheetStream.Read()) != null)
            {
                switch (record)
                {
                    ...
                    case CellRecord cell when inSheetData:
                        ...
                        var numberFormatIndex = Workbook.GetNumberFormatFromXF(cell.XfIndex);
                        cells.Add(new Cell(cell.ColumnIndex, numberFormatIndex, ConvertCellValue(cell.Value, numberFormatIndex)));
                        break;
                }
            }
